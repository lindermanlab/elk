"""
deer.py
code taken from original DEER codebase (Lim et al '24)
at: https://github.com/machine-discovery/deer
based on: 17b0b625d3413cb3251418980fb78916e5dacfaa
from 1/18/24
modifications for benchmarking, and implementation of quasi-DEER
"""
...
def deer_iteration_bwd(
    ...
    if gts is None:
        ...
        if quasi:
            # note that this backwards pass for quasi-DEER is computationally efficient, but does not result in exact gradients. We discuss in more detail, including the empirical success of this approach in Section 6.2
    ...
    _, grad_rhs, grad_inv_lin_params = inv_lin_dual(grad_yt[0])  # such a hack...
...
# helper function to create the gru function for deer experiments


def make_gru_function(nh=32, ninput=1, seed=0):
    """
    For given specificaiton, returns a GRU cell and an initial hidden state

    Args:
      nh: dimension of hidden state
      ninput: dimension of input
    ...
    Returns:
      ...
      init_state: packed_state
      gru: the actual gru cell
      parmas: the parameters of the gru cel

    Notes:
      * let's turn of bach_size for now
    """
    gru = flax.linen.GRUCell(features=nh)
    key = jr.PRNGKey(seed)
    key, *subkey = jr.split(key, 3)
    # init_state = gru.initialize_carry(subkey[0], (1, nh))  # (batch_size, nh)
    init_state = gru.initialize_carry(subkey[0], (nh,))  # (nh,) # all zeros LOL

    # drivers = jax.random.normal(subkey[1], (nsequence, 1, nh))  # (nsequence, batch_size, nh)
    params = gru.init(key, init_state, jnp.zeros(ninput))

    f = lambda state, driver: gru.apply(params, state, driver)[
        0
    ]  # function from state and driver to create the next state

    return f, init_state, gru, params
...
def gru_diagonal_derivative(carry, inputs, params):
    ...
    * eqx.nn.GRUCell is quite different :(
    ...
    )  # + hr['bias'] # why doesn't their formula have ir['bias']?
    r = sigmoid(ract)
    # print(f"r is {r}")

    zact = (
        iz["kernel"].T @ inputs + iz["bias"] + hz["kernel"].T @ carry
    )  # + hz['bias']  # why doesn't their formula have iz['bias']?
    z = sigmoid(zact)

    n_act = in_["kernel"].T @ inputs + in_["bias"] + r * rcomp
    n = tanh(n_act)

    # get the derivative terms (are there more stable ways to compute these derivatives?)
